...
|---|---------|--------------------|------------|
| 1 | Autenticación: registro e inicio de sesión | Sin saber qué usuario está activo, ninguna tarea se puede asignar a nadie. Es el prerrequisito de todo lo demás. | — |
| 2 | Registro de tarea con pilar fijo (Uni / Tech / Emprendimiento) | Sin esto no hay datos que auditar. Es el input mínimo de todo el sistema. | #1 |
| 3 | Registro de duración/tiempo por tarea | El balance se mide en tiempo invertido, no en cantidad de tareas. Sin duración, el % por pilar no se puede calcular. | #2 |
| 4 | Vista de balance: % de tiempo invertido por pilar | Es el diferenciador explícito frente a un to-do list: "no mide cuántas tareas, mide balance de energía". | #2, #3 |
| 5 | Alerta de "Pérdida de Tracción" cuando Emprendimiento llega a 0% | Es la pieza que convierte el dato en acción — el argumento central frente al evaluador. | #4 |

Con estas 5 piezas ya existe el producto completo: se identifica al usuario, se registra, se audita, se alerta. Cualquier feature adicional en esta fase retrasa la fecha en que alguien lo usa de verdad.
